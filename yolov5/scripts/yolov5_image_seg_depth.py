# !/usr/bin/env python
# 实时显示摄像头的RGB图像，并使用YOLOv5分割物体,显示分割物体的2D中心坐标和深度信息
import rospy
                ...
                for mask, conf, cls in zip(masks, det[:, 4], det[:, 5]):
                    ...
                    if non_zero_coords is not None:
                        ...
                        if self.depth_image is not None:
                            ...
                            depth_label = f'Depth: {depth_value_m:.3f}m'
                            cv2.putText(cv_image, depth_label, (center[0], center[1] + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
                            print(f'深度: {depth_value_m:.3f}m')
                    else:
                        print("掩码中没有非零像素")
