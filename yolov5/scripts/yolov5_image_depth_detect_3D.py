#!/usr/bin/env python
# 实时显示摄像头的RGB图像，并使用YOLOv5检测物体，显示物体的2D中心坐标，同时获取物体中心的深度信息，转化成物体的3D坐标
import rospy
...
class VideoSaver:
    def __init__(self):
        ...
        # 相机内参矩阵
        self.fx = 570.3422047415297  # 焦距在x方向上的像素值
        self.fy = 570.3422047415297  # 焦距在y方向上的像素值
        self.cx = 319.5  # 主点x坐标
        self.cy = 239.5  # 主点y坐标
    ...
    def callback(self, data):
        ...
        for *xyxy, conf, cls in results.xyxy[0]:
            ...
            center_x = (x1 + x2) // 2
            center_y = (y1 + y2) // 2
            coordinates_3d = self.get_3d_coordinates(center_x, center_y)
            class_name = self.model.names[int(cls)]  # 获取类别名称
            if coordinates_3d is not None:
                rospy.loginfo(f"Detected object: Class={class_name}, Confidence={conf:.2f}, Center=({center_x}, {center_y}), 3D Coordinates=({coordinates_3d[0]:.2f}, {coordinates_3d[1]:.2f}, {coordinates_3d[2]:.2f})")
            else:
                rospy.logwarn(f"Failed to get 3D coordinates for object at Center=({center_x}, {center_y})")
